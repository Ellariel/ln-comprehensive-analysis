...
results_file = os.path.join(results_dir, f"powlaw_metrics.pkl")
metric_file = os.path.join(results_dir, f"powlaw_estimates.csv")
...
for batch in tqdm(batched(graphs.fname, args.batch_size), total=math.ceil(len(graphs.fname)/args.batch_size)):
...
metrics = pd.DataFrame(metrics, 
            columns=['timestamp', 'powlaw_alpha', 'powlaw_k', 'powlaw_rsq'])
