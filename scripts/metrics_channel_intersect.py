...
results_file = os.path.join(results_dir, f"channel_intersect.{args.alg}.{directed}.csv")
...
def route_and_count(g1, g2, sample, alg="DEF", seed=13):
    ...
    for u, v in sample:
        try:
            base_path_length = len(get_shortest_path(g1, u, v, 100, alg))
            if base_path_length > 0:
                total_paths += 1
                new_path_length = len(get_shortest_path(g2, u, v, 100, alg))
                if new_path_length > 0 and base_path_length >= new_path_length:
                    matched_paths += 1
        except:  # noqa: E722
            pass
...
def edges_intersection_rate(
    ...
    for sample in batched(combinations(common_nodes, 2), sample_size):
        tasks.append(
            route_and_count.remote(ray.put(g1), ray.put(g2), sample, alg, seed)
        )
        task_count += 1
