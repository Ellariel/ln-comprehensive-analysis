...
discrete = "discrete" if args.discrete else "undiscrete"
results_file = os.path.join(results_dir, f"powerlaw.{discrete}.{directed}.pkl")
metric_file = os.path.join(results_dir, f"powerlaw.{discrete}.{directed}.csv")
...
def proccess_graph(g, seed=13):
    ...
    s = get_stamp(g)
    g = nx.read_gml(g)
    r = fit_powerlaw(g, discrete=bool(args.discrete))
    r.update({"timestamp": s})
    return r
