...
results_file = os.path.join(results_dir, f"tx_metrics.{args.alg}.{directed}.pkl")
...
for g in tqdm(graphs.fname):
    ...
    if timestamp not in results:
        with open(
            os.path.join(data_dir, f"{timestamp}.txs.{directed}.pkl"), "rb"
        ) as handle:
            g, txset = pickle.load(handle)
        assert len(g.nodes) > 500
        assert len(txset) >= 5000
        paths = []
        for tx in tqdm(txset[:5000], leave=False):
            paths += get_shortest_path(g, tx[0], tx[1], tx[2], args.alg)
        results[timestamp] = cum(paths)
        # print(f"{timestamp}: {len(results[timestamp])}")
